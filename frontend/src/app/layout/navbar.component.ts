          ...
          <nav class="order-3 flex w-full items-center gap-1 overflow-x-auto md:order-none md:w-auto">
            @for (item of menu; track item.path) {
              <a
                ...
                routerLinkActive="bg-zinc-900 text-white shadow-xs"
                class="whitespace-nowrap rounded-lg px-3 py-1.5 text-xs font-medium text-zinc-600 transition hover:bg-zinc-100 hover:text-zinc-900"
              >
                {{ item.label }}
              </a>
            }
